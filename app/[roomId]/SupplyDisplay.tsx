// --- FILE: app/[roomId]/SupplyDisplay.tsx (並び替え機能を追加) ---
'use client';
...
export default function SupplyDisplay({ initialCards }: { initialCards: CardType[] }) {
  ...
  const sortedItems = useMemo(() => { return [...displayItems].sort((a, b) => { const aIsCard = isCard(a); const bIsCard = isCard(b); const aCost = aIsCard ? a.cost : -1; const bCost = bIsCard ? b.cost : -1; const aName = aIsCard ? a.name : '空き枠'; const bName = bIsCard ? b.name : '空き枠'; if (sortBy === 'cost') return aCost - bCost || aName.localeCompare(bName, 'ja'); if (sortBy === 'name') return aName.localeCompare(bName, 'ja'); return 0; }); }, [displayItems, sortBy]);
  ...
  const handleReplaceWithPlaceholder = () => { if (selectedIds.size === 0) return; setDisplayItems(prevItems => prevItems.map(item => (selectedIds.has(item.id) ? { id: crypto.randomUUID(), type: 'placeholder' } : item))); setSelectedIds(new Set()); };
  const handleRerollSelected = async () => { if (selectedIds.size === 0) return; setIsRerolling(true); setError(null); const selectedExpansionsArray = Array.from(selectedExpansions); if (selectedExpansionsArray.length === 0) { setError("再抽選に使用する拡張セットを少なくとも1つ選択してください。"); setIsRerolling(false); return; } try { const currentCardIds = displayItems.filter(isCard).map(c => c.id); const { data: availableCards, error: fetchError } = await supabase.from('cards').select('id').in('expansion', selectedExpansionsArray).not('id', 'in', `(${currentCardIds.join(',') || '""'})`); if (fetchError) throw new Error(fetchError.message); if (!availableCards || availableCards.length < selectedIds.size) throw new Error('選択された拡張セットに、再抽選するためのカードが不足しています。'); const newCardIds = shuffle(availableCards).slice(0, selectedIds.size).map(c => c.id); const { data: newFullCards, error: newCardsError } = await supabase.from('cards').select('*').in('id', newCardIds); if (newCardsError || !newFullCards) throw new Error(newCardsError?.message || 'カード取得失敗'); const newCardsQueue = [...newFullCards]; const newDisplayItems = displayItems.map(item => (selectedIds.has(item.id) ? newCardsQueue.shift()! : item)); setDisplayItems(newDisplayItems); setNewlyAddedCardIds(new Set(newFullCards.map(c => c.id))); setSelectedIds(new Set()); } catch (err: any) { setError(err.message); } finally { setIsRerolling(false); } };
      ...
      <main className="flex-1 transition-all duration-300 ease-in-out">
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
          <div className="flex justify-between items-center mb-2 pt-14">
            <div><h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">生成されたサプライ</h1><p className="text-gray-500 dark:text-gray-400 mt-1">王国カード ({displayItems.filter(isCard).length}種類)</p></div>
            <div className="flex items-center space-x-2"><CopyUrlButton /><Link href="/" className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">新しく生成</Link></div>
          </div>
