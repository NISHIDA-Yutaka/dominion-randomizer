// --- FILE: app/[roomId]/SupplyDisplay.tsx (最終修正案) ---
'use client';

import { useState, useMemo, useEffect, useRef } from 'react';
import Link from 'next/link';
import { Card as CardType, Room } from '../../types';
import { RealtimeChannel } from '@supabase/supabase-js'; // RealtimeChannelをインポート
...
// ★ 変更点1: コンポーネントの外に、チャンネルを管理するオブジェクトを作成
// これにより、Reactの再レンダリングの影響を受けずにチャンネルを保持できる
...
export default function SupplyDisplay({ initialCards, roomId }: { initialCards: CardType[], roomId: string }) {
  ...
  // ★ 変更点2: リアルタイム購読のuseEffectを新しい方式に刷新
  useEffect(() => {
    // このルームID用のチャンネルがまだ作成されていなければ、新しく作成する
    ...
    // ★ 変更点3: クリーンアップ処理を削除
    // 意図的にクリーンアップ（removeChannel）を削除し、ストリクトモードで接続が切断されないようにする
    // これにより、ユーザーがページを離れるまで接続が維持される
  }, [roomId]);
  ...
  const sortedItems = useMemo(() => { return [...displayItems].sort((a, b) => { const aIsCard = isCard(a); const bIsCard = isCard(b); const aExpansionIndex = aIsCard ? EXPANSION_ORDER.indexOf(a.expansion) : Infinity; const bExpansionIndex = bIsCard ? EXPANSION_ORDER.indexOf(b.expansion) : Infinity; const aCost = aIsCard ? a.cost : -1; const bCost = bIsCard ? b.cost : -1; const aName = aIsCard ? a.name : '空き枠'; const bName = bIsCard ? b.name : '空き枠'; if (sortBy === 'expansion') { const expansionCompare = aExpansionIndex - bExpansionIndex; if (expansionCompare !== 0) return expansionCompare; return aCost - bCost || aName.localeCompare(bName, 'ja'); } return aCost - bCost || aName.localeCompare(bName, 'ja'); }); }, [displayItems, sortBy]);
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen">
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-4/5 max-w-lg bg-gray-50 dark:bg-gray-800 transform transition-transform duration-300 ease-in-out shadow-2xl ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}><Sidebar allCards={allCards} allExpansions={allExpansions} selectedExpansions={selectedExpansions} onExpansionChange={handleExpansionChange} /></div>
      <main className="flex-1"><div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8"><div className="flex justify-between items-center mb-2 pt-14"><div><h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">生成されたサプライ</h1><p className="text-gray-500 dark:text-gray-400 mt-1">王国カード ({cards.length}種類)</p></div><div className="flex items-center space-x-2"><CopyUrlButton /><Link href="/" className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">新しく生成</Link></div></div><div className="my-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow flex flex-wrap items-center justify-between gap-4"><div><span className="font-bold mr-3">並び替え:</span><button onClick={() => setSortBy('cost')} className={`px-3 py-1 rounded ${sortBy === 'cost' ? 'bg-indigo-600 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>コスト</button><button onClick={() => setSortBy('expansion')} className={`ml-2 px-3 py-1 rounded ${sortBy === 'expansion' ? 'bg-indigo-600 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>拡張</button></div><div className="flex items-center gap-2"><button onClick={handleRerollSelected} disabled={selectedIds.size === 0 || isRerolling} className="px-4 py-2 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center">{isRerolling ? <LoadingSpinner /> : <ArrowPathIcon className="h-5 w-5 mr-2" />}再抽選</button><button onClick={handleReplaceWithPlaceholder} disabled={selectedIds.size === 0 || Array.from(selectedIds).every(id => !cards.find(c => c.id === id)) } className="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center"><TrashIcon className="h-5 w-5 mr-2" />削除</button></div></div>{error && <p className="mt-2 text-center text-red-500 font-bold">{error}</p>}<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">{sortedItems.map((item) => isCard(item) ? (<CardComponent key={item.id} card={item} isSelected={selectedIds.has(item.id)} onToggleSelect={handleToggleSelect} isNew={newlyAddedCardIds.has(item.id)}/>) : (<PlaceholderCard key={item.id} placeholder={item} isSelected={selectedIds.has(item.id)} onToggleSelect={handleToggleSelect}/>))}</div></div></main>
    </div>
  );
}